...
def sigmoid_ouput_to-derivative(output):
...
for j in range(10000):
	...
	for position in range(binary_dim):
		...
		layer_1 = sigmoid(np.dot(X,synapse_0) + np.dot(layer_1_values[-1],synapse_1))
		layer_2 = sigmoid(np.dot(layer_1,synapse_1))
		layer_2_error = y - layer_2
		layer_2_deltas.append((layer_2_error)*sigmoid_ouput_to-derivative(layer_2))
		overallError += ap.abs(layer_2_error[0])
	...
	for potition in range(binary_dim):
		...
		layer_2_delta = (future_layer_1_delta.dot(synapse_h.T)+layer_2_delta.dot(synapse_1.T))*
						sigmoid_ouput_to-derivative(layer_1)

		#updating all the weights to try again

		synapse_1_update += np.atleast_2d(layer_1).T.dot(layer_2_delta)
		synapse_h_update += np.atleast_2d(prev_layer_1).T.dot(layer_2_delta)
		synapse_0_update += np.atleast_2d()
